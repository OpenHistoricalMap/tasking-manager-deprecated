(function () {
    ...
    function editorService($window, $location, mapService, configService) {
        ...
        function launchPotlatch2Editor(centroid){
            var base = 'http://www.openhsitoricalmap.org/edit?editor=potlatch2';
            var zoom = mapService.getOSMMap().getView().getZoom();
        ...
        function launchIdEditor(centroid, changesetComment, imageryUrl, projectId, taskId){
            var base = 'http://www.openhistoricalmap.org/edit?editor=id&';
            var zoom = mapService.getOSMMap().getView().getZoom();
            var url = base + '#map=' +
         ...
         * @returns {boolean} Did JOSM Respond successfully
         */
        function sendJOSMCmd(endpoint, params) {
            ...
            // preflighting the GET request with an OPTIONS requests due to CORS.
            // JOSM does not support the OPTIONS requests
            // After some time, we were unable to find a way to control the header to stop the preflighting
            // The workaround is as you see here, to use XMLHttpRequest in synchronous mode
